...
class TopKSAE(base_sae.BaseSAE):
    ...
    @torch.no_grad()
    def normalize_decoder_topk(self):
        """
        Normalize decoder weights for Topk activation.
        Adjusts encoder weights and biases to maintain output stability.
        """
        original_dtype = self.W_dec.dtype
        self.to(dtype=torch.float32)

        # Calculate norms and normalize decoder weights
        norms = torch.norm(self.W_dec, dim=1, keepdim=True)
        self.W_dec.data /= norms
        
        # Adjust encoder weights and biases
        norms = norms.squeeze(1)  # Remove extra dimension for broadcasting
        self.W_enc *= norms
        self.b_enc *= norms
        ...
        if not torch.allclose(new_norms, torch.ones_like(new_norms), atol=tolerance):
            max_diff = (new_norms - 1).abs().max().item()
            print(f"Warning: Max norm deviation after normalization: {max_diff}")

        # Test output consistency with Topk activation
        test_input = torch.randn(10, self.cfg.d_in).to(self.device)
        with torch.no_grad():
            initial_output = self.forward_topk(test_input)  # 使用Topk前向传播
            new_output = self.forward_topk(test_input)
        
        max_diff = (initial_output - new_output).abs().max().item()
        print(f"Output change after normalization: {max_diff}")
