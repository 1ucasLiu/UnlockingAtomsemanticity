...
class BatchTopKTrainer(SAETrainer):
    ...
    def loss(self, x, step=None, logging=False):
        ...
        cos_sim,info_nce,afa,hsic_loss = None, None, None, None

        if self.loss_type =="fvu_loss":
            loss = fvu + self.auxk_alpha * auxk_loss 
        elif self.loss_type == "hsic_loss":
            hsic_loss = self.batch_sparse_linear_hsic_full(self.random_sample(f,2000)[0].T)
        ...
        if not logging:
            ...
            return namedtuple("LossLog", ["x", "x_hat", "f", "losses"])(
                ...
                {"l2_loss": l2_loss.item(), "auxk_loss": auxk_loss.item(), "loss": loss.item(),
                 "fvu":fvu,
                **({"hsic_loss": hsic_loss.item()} if hsic_loss is not None else {}),
                **({"cos_sim": cos_sim.item()} if cos_sim is not None else {}),
                **({"info_nce_loss": info_nce.item()} if info_nce is not None else {}),
                 },
            )
    ...
    def batch_sparse_linear_hsic_full(self,X):
