...
def run_eval(
    ...
    if "it" not in config.model_name:
        ...
        # CODE CHANGED 
        # raise ValueError("Model should be instruct tuned")    源代码
...
if __name__ == "__main__":
    ...
    random_seed = 168 #126 #32 #72# 52 #42

    output_folder = f"eval_results_gemma-final/test_unlearning_seed/unlearning_seed_{random_seed}"   # eval_results_relu
    ...
    for _,dirnames,filenames in os.walk(rootdir):
        for sae_name in dirnames:
            if not sae_name.startswith("100"):
                continue
            print(sae_name)
            if sae_type == "topk":
            ...
            selected_saes = [(f"{sae_name}_{filename}", sae)] 
            ...
            config.llm_batch_size = 4 #32 #activation_collection.LLM_NAME_TO_BATCH_SIZE[config.model_name]
