...
class AutoEncoderTopK(Dictionary, nn.Module):
    ...
    def encode(self, x: t.Tensor, return_topk: bool = False, use_threshold: bool = False):
        ...
        origin_encode_res = self.encoder(x - self.b_dec)

        post_relu_feat_acts_BF = nn.functional.relu(origin_encode_res)   
        ...
        if return_topk:
            return encoded_acts_BF, tops_acts_BK, top_indices_BK, post_relu_feat_acts_BF, origin_encode_res 
        else:
            return encoded_acts_BF
...
class TopKTrainer(SAETrainer):
    ...
    def __init__(
        ...
        loss_type:Optional[str] = "standard",
        constraint_type:Optional[str] = "sample",
        term_coff:Optional[int] = 100,
    ...
    def loss(self, x, step=None, logging=False):
        ...
        f, top_acts_BK, top_indices_BK, post_relu_acts_BF, origin_encode_res = self.ae.encode(
        ...
        mse = e.pow(2).mean()
        total_variance = (x - x.mean(0)).pow(2).mean()
        fvu = mse / total_variance

        hsic_sample_dim = self.batch_linear_hsic(self.random_sample(f,2000)[0].T)  
        hsic_loss = hsic_sample_dim
        if self.loss_type =="fvu_loss":
            loss = fvu + self.auxk_alpha * auxk_loss 
            
        elif self.loss_type == "hsic_loss":
            loss = fvu + self.auxk_alpha * auxk_loss + self.term_coff*hsic_loss
            
        elif self.loss_type == "linear_loss":
            sampled_after_topk, indices = self.random_sample(f,2000)
            cos_sim = self.linear_term(sampled_after_topk.T)    
            loss = fvu + self.auxk_alpha * auxk_loss + self.term_coff*cos_sim
            
        elif self.loss_type == "nce_loss":
            sampled_after_topk, indices = self.random_sample(f,2000)   
            info_nce = self.info_nce_loss(sampled_after_topk,sampled_after_topk)    

            loss = fvu + self.auxk_alpha * auxk_loss + self.term_coff*info_nce 
    ...
    def calculate_pmi_info_nce_loss(self):
        scores = t.matmul(self.ae.decoder.weight, self.ae.decoder.weight.T) 
        loss = t.norm(scores * (1 - t.eye(scores.size(0),device=scores.device)), p="fro")
        return loss
    

    def batch_linear_hsic(self,X):
        n, d = X.shape
        const = (n - 1) ** 2  
        
        col_means = X.mean(dim=0)
        X_centered = X - col_means.view(1, -1)  # (n, d)

        inner_product = X_centered.T @ X_centered  # (d, d)
        # HSIC 
        hsic_matrix = (inner_product ** 2) / const  # Frobenius 

        return t.mean(hsic_matrix)
 
        
    def random_sample(self, x, sampled_rows=2000):
        total_rows, total_cols = x.size()

        indices = t.randperm(total_rows)[:sampled_rows]

        sub_mat = x[indices]
        return sub_mat, indices


    def info_nce_loss(self,query, key, temperature=0.1):
        """
        query: (batch_size, d)
        key:   (batch_size, d)  # 正样本匹配
        """
        batch_size = query.shape[0]
        query = F.normalize(query, dim=1)
        key = F.normalize(key, dim=1)

        logits = t.matmul(query, key.T) / temperature
        labels = t.arange(batch_size, device=logits.device)

        loss = F.cross_entropy(logits, labels)
        return loss
    
    
    def linear_term(self, matrix, eps=1e-8):
        matrix = matrix.to_dense() if matrix.is_sparse else matrix
        matrix_centered = matrix - matrix.mean(dim=0, keepdim=True) 

        norms = t.norm(matrix_centered, p=2, dim=0, keepdim=True).clamp(min=eps)
        matrix_normalized = matrix_centered / norms                  

        correlation_matrix = t.mm(matrix_normalized.t(), matrix_normalized)
        D = correlation_matrix.size(0)

        off_diag_mask = ~t.eye(D, dtype=t.bool, device=correlation_matrix.device)
        off_diag_mean = correlation_matrix[off_diag_mask].mean()
