...
TRAINER_COLORS = {
    ...
    # TopK
    "fvu_only_500M":(250/255,134/255,0),#"black",
    "100fvu_hsic_500M":(254/255,183/255,5/255),#"#42B540B2",
...
def plot_results(
    ...
    titles = {'Absorption':"Feature Absorption",
              ...
              'Sparse Probing':"Sparse Probing"}
    y_labels = custom_metric_name
    title_2var = custom_metric_name
    for title,full_title in titles.items():
        if title in custom_metric_name:
            title_2var = full_title
            break
    if title_2var == 'RAVEL':
        y_labels = "Disentanglement Score"

#    if "top" in title_2var:
        #names = title_2var.split('_')
        #title_2var, y_labels = names[0],names[1].replace("_","")+" Accuracy"
    # else:   
    #     y_labels =custom_metric_name+' score'
    y_labels ='Score'
    fig = plot_2var_graph(
        eval_results,
        custom_metric,
        y_label=y_labels,#custom_metric_name,
        x_label='L0',
        title=title_2var,
        output_filename=f"{image_base_name}_2var_sae_type.png",
        trainer_markers=trainer_markers,
...
def plot_2var_graph(
    ...
    for trainer, marker in trainer_markers.items():
        # Filter data for this trainer
        #trainer_data = {k: v for k, v in results.items() if v["sae_class"] == trainer}
        trainer_data = {k: v for k, v in results.items() if v["loss_type"] == trainer}
        for k,v in results.items():
            if v["loss_type"] == trainer:
                print(trainer)
        if not trainer_data:
            continue  # Skip this trainer if no data points
        ...
        custom_metric_values = [data[custom_metric] for data in trainer_data.values()]
        if custom_metric in ['mean_absorption_fraction_score',"mean_full_absorption_score"]:
            # 1-absorption score
            custom_metric_values = [ 1-data[custom_metric] for data in trainer_data.values()]
...
def plot_2var_graph_K_l0(
    results: dict[str, dict[str, float]],
    custom_metric: str,
    title: str = "k vs Custom Metric",
    y_label: str = "Custom Metric",
    xlims: tuple[float, float] | None = None,
    ylims: tuple[float, float] | None = None,
    output_filename: str | None = None,
    ...
    # tick_locations = [10**i for i in range(int(np.floor(np.log10(xmin))), int(np.ceil(np.log10(xmax))) + 1)]
    # ax.xaxis.set_major_locator(FixedLocator(tick_locations))
    # ax.xaxis.set_major_formatter(FuncFormatter(lambda x, pos: f'$10^{{{int(np.log10(x))}}}$'))
