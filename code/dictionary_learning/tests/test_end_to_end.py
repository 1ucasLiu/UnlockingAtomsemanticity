...
def test_sae_training():
    ...
    context_length = 1024
    llm_batch_size = 32  # Fits on a 24GB GPU
    sae_batch_size = 8192
...
def sae_training():
    ...
    context_length = 1024
    llm_batch_size = 64  # Fits on a 24GB GPU
    sae_batch_size = 8192
    num_contexts_per_sae_batch = sae_batch_size // context_length
    ...
    #generator = disk_generator(DATASET_NAME)
    activation_buffer = ActivationBuffer(
        generator,
        model,
        submodule,
        ...
        [
            {
                ...
                "steps": steps,  # when when does training end
                "seed": RANDOM_SEED,
                "wandb_name": f"TopKTrainer-{k}k-{MODEL_NAME}-{submodule_name}",
                "device": DEVICE,
                "layer": LAYER,
                "lm_name": MODEL_NAME,
                "submodule_name": submodule_name,
    ...
    for folder in folders:
        dictionary, config = d(folder, DEVICE)

        assert dictionary is not None
        assert config is not None

def disk_generator(dataset_name):
